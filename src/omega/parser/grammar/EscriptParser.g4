...
variableDeclarationInitializer
    : ':=' expression
    | '=' expression { self.notifyErrorListeners("Unexpected token: '='. Did you mean := for assign?\n"); }
    | ARRAY
    ;
...
expression
    ...
    | expression bop=('<=' | '>=' | '>' | '<') expression
    | expression bop='=' { self.notifyErrorListeners("Deprecated '=' found: did you mean '==' or ':='?\n"); } expression
    | expression bop=('==' | '!=' | '<>') expression
...
bareArrayInitializer
    ...
    | LBRACE expressionList? ',' RBRACE {self.notifyErrorListeners("Expected expression following comma before right-brace in array initializer list");}
    ;
...
structInitializer
    : '{' structInitializerExpressionList? '}'
    | '{' structInitializerExpressionList? ',' '}' {self.notifyErrorListeners("Expected expression following comma before right-brace in struct initializer list");}
    ;
...
dictInitializer
    : '{' dictInitializerExpressionList? '}'
    | '{' dictInitializerExpressionList? ',' '}' {self.notifyErrorListeners("Expected expression following comma before right-brace in dictionary initializer list");}
    ;

arrayInitializer
    : '{' expressionList? '}'
    | '{' expressionList? ',' '}' {self.notifyErrorListeners("Expected expression following comma before right-brace in array initializer list");}
    | '(' expressionList? ')'
    | '(' expressionList? ',' ')' {self.notifyErrorListeners("Expected expression following comma before right-paren in array initializer list");}
    ;
